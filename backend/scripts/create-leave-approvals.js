...
async function createWorkflowApprovalsForLeaveRequests() {
  console.log(
    '🔄 Creating workflow approvals for existing leave requests...\n',
  );
  ...
  try {
    ...
    console.log(
      `📋 Found ${pendingLeaveRequests.length} pending leave requests\n`,
    );

    if (pendingLeaveRequests.length === 0) {
      console.log(
        'ℹ️  No pending leave requests found. Creating a sample one...\n',
      );
      ...
      pendingLeaveRequests.push(newLeaveRequest);
      console.log(
        `✅ Created sample leave request for ${employee.firstName} ${employee.lastName}\n`,
      );
    }
